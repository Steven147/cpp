...
class vec
{
    friend istream & operator >> (istream & is, vec & obj);
...
public:
    ...
    //加法重载
    vec operator + (const vec & rightV)
    {
        vec result = rightV; //调用了一次拷贝构造函数
...
//输入重载，【输入方案不好，无法按需输入】
istream & operator >> (istream & is, vec & obj)
{
...
int main()
{
    ...
    cout << "v1 - v2 : " << v1 - v2 << endl;
    cout << "v1 * v2 : " << v1 * v2 << endl;
    cout << "input vec（单个空格分开数字，最后一个数字后直接回车，维数只能为3）: " << endl;
    cin >> v3;
    cout << "v3' : " << v3 << endl;
